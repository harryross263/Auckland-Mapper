...
/**
 * Location A Location is a point in a 2D coordinate system, with increasing x
 * from west to east and increasing y from south to north (ordinary mathematical
 * coordinates). Locations are represented with two doubles, (with an
 * unspecified length unit - could be kilometers, for example).
 * 
 * Location provides methods for converting between Locations and Points
 * representing pixel positions on a window/screen. A Point is represented by
 * two integers representing screen/window positions (x to the right and y
 * down).
 * 
 * The conversion needs - an origin Location [a Location at the origin will be
 * converted to the point (0,0)] - a scale specifying how many pixels per length
 * unit. Typically the scale will be ( windowSize /(maxLocation - minLocation))
 * 
 * @author unknown
 */
public class Location {
	...
	// --------- CONVERSION methods ------------------------
	/**
	 * Return a location specified by a point on the window, given an origin and
	 * a scale. Note the vertical direction is inverted. invoked as
	 * Location.newFromPoint(point, orig, scale);
	 */
	...
	private static final double centerLat = -36.847622; // the center of
														// Auckland City!
	private static final double centerLon = 174.763444; // according to Google
														// Maps
	/**
	 * Return a location specified by a latitude and a longitude
	 */
	private static final double scaleLat = 111.0; // how many kilometers per
													// degree.
	...
	public static Location newFromLatLon(double lat, double lon) {
		...
		double x = (lon - centerLon)
				* (scaleLat * Math.cos((lat - centerLat) * degToRad));
	...
	public static void main(String[] arguments) {
		System.out.println("new Location(10.5, 42.0) -> "
				+ new Location(10.5, 42.0));
		...
		// the origin of the window corresponds to location (west, north)=(20.0,
		// 200.0)
		...
		System.out.printf("%norigin: loc:%s%nscale: %.3f%nplace: %s%n", origin,
				scale, place);
		...
		System.out.printf("%norigin: loc:%s%nscale: %.3f%nmouse: %s%n", origin,
				scale, mouse);

		// work out which location this corresponds to:
		System.out.printf(
				"%nLocation.newFromPoint(mouse, origin, scale) -> %s%n",
				Location.newFromPoint(mouse, origin, scale));

		// work out location from lat/long:
		System.out.printf(
				"%nLocation.newFromLatLon(-37.8899, 174.724) -> loc:%s%n",
